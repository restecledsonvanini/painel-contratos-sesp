...
export async function listAlteracoes(req: Request, res: Response) {
  return res.status(200).json(await alteracaoService.list(routeParam(req, 'id'), req));
}

export async function getAlteracao(req: Request, res: Response) {
  return res
    .status(200)
    .json(await alteracaoService.get(routeParam(req, 'id'), routeParam(req, 'alteracaoId'), req));
}
...
export async function createAlteracao(req: Request, res: Response) {
  ...
  return res.status(201).json(await alteracaoService.create(routeParam(req, 'id'), parsed, req));
}
...
export async function deleteAlteracao(req: Request, res: Response) {
  return res
    .status(200)
    .json(
      await alteracaoService.remove(routeParam(req, 'id'), routeParam(req, 'alteracaoId'), req),
    );
}

export async function simularAlteracao(req: Request, res: Response) {
  const parsed = AlteracaoSimularSchema.parse(req.body);
  return res.status(200).json(await alteracaoService.simular(routeParam(req, 'id'), parsed, req));
}
