...
export default function UsuariosPage() {
  ...
  const list = useQuery({
    queryKey: ['usuarios'],
    queryFn: async () => (await http.get<Usuario[]>('/usuarios')).data,
  });
    ...
    mutationFn: async () =>
      (
        await http.post('/usuarios', {
          ...
          password,
          role,
          orgaoId: orgaoId && orgaoId !== '__none__' ? orgaoId : null,
        })
      ).data,
    onSuccess: () => {
  ...
  const patch = useMutation({
    mutationFn: async (input: { id: string; data: Record<string, unknown> }) =>
      (await http.patch(`/usuarios/${input.id}`, input.data)).data,
    onSuccess: () => {
  ...
  return (
    <Page
      title="Usuários"
      description="Papéis de acesso, órgão de escopo e ativação. Somente ADMIN."
    >
      <Card variant="bordered" className="mb-4 space-y-3 p-4">
        ...
        <div className="grid grid-cols-1 gap-3 md:grid-cols-2 xl:grid-cols-3">
          ...
          <Select
            ...
            onChange={setOrgaoId}
            options={orgaoOptions.map((o) => ({ id: o.value || '__none__', label: o.label }))}
                ...
                <TableRow key={u.id}>
                  ...
                  <TableCell>
                    <select
                      ...
                      value={u.role}
                      onChange={(e) =>
                        patch.mutate({ id: u.id, data: { role: e.target.value } })
                      }
                  ...
                  <TableCell>{u.orgao?.sigla || '—'}</TableCell>
                  <TableCell>
